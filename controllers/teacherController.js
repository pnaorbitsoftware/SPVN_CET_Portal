...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [testCount, questionCount, studentsTested] = await Promise.all([
      ...
      Result.count({ include: [{ model: Test, as: 'test', where: { createdBy: teacherId }, required: true }] }),
    ]);
    const recentTests = await Test.findAll({
      where: { createdBy: teacherId }, order: [['createdAt','DESC']], limit: 5,
      include: [{ model: Group, as: 'groups', through: { attributes: [] } }],
    });
    res.render('teacher/dashboard', { title: 'Teacher Dashboard', stats: { testCount, questionCount, studentsTested }, recentTests });
  } catch (e) { req.flash('error','Failed to load dashboard.'); res.redirect('/auth/login'); }
};
...
exports.getQuestions = async (req, res) => {
  try {
    ...
    const { count, rows: questions } = await Question.findAndCountAll({ where, order: [['createdAt','DESC']], limit, offset });
  ...
  } catch (e) { req.flash('error','Failed to load questions.'); res.redirect('/teacher/dashboard'); }
};
  ...
  try {
    ...
    if (req.files) {
      ...
      if (req.files.explanationImage) {
        explanationImage = await processQuestionImage(req.files.explanationImage, `exp_${Date.now()}`);
      }
    }

    await Question.create({
      question, optionA, optionB, optionC, optionD,
      correctAnswer, subject, difficulty, marks: parseFloat(marks)||1,
      explanation, topic, questionImage, explanationImage,
      createdBy: req.session.user.id,
    });
    req.flash('success','Question added successfully.');
    res.redirect('/teacher/questions');
  } catch (e) { req.flash('error','Failed to create question: '+e.message); res.redirect('/teacher/questions'); }
};

exports.bulkImportQuestions = async (req, res) => {
  try {
    if (!req.files?.csvFile) { req.flash('error','No file uploaded.'); return res.redirect('/teacher/questions'); }
    const workbook = xlsx.read(req.files.csvFile.data, { type: 'buffer' });
    ...
    res.redirect('/teacher/questions');
  } catch (e) { req.flash('error','Import failed.'); res.redirect('/teacher/questions'); }
};
...
exports.deleteQuestion = async (req, res) => {
  try {
    ...
    res.redirect('/teacher/questions');
  } catch (e) { req.flash('error','Failed.'); res.redirect('/teacher/questions'); }
};
...
exports.getTests = async (req, res) => {
  ...
  } catch (e) { req.flash('error','Failed.'); res.redirect('/teacher/dashboard'); }
};
...
exports.getCreateTest = async (req, res) => {
  ...
  } catch (e) { req.flash('error','Failed.'); res.redirect('/teacher/tests'); }
};
...
exports.createTest = async (req, res) => {
  ...
  } catch (e) { req.flash('error','Failed: '+e.message); res.redirect('/teacher/tests/create'); }
};
...
exports.getTestDetail = async (req, res) => {
  try {
    ...
    if (!test) { req.flash('error','Not found.'); return res.redirect('/teacher/tests'); }
  ...
  try {
    ...
    for (const tg of testGroups) {
      ...
      for (const m of members) {
        await Notification.create({ userId: m.userId, title: 'New Exam Published', message: `"${test.title}" is now available. Duration: ${test.duration} mins.`, type: 'exam', link: '/student/tests' });
      }
    }
