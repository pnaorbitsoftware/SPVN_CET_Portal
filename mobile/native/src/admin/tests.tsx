...
const blank = (): TestPayload => ({ title: '', description: '', timingMode:'PERSONAL_DURATION', duration: 180, negativeMarking: 0.25, passingMarks: null, questionIds: [], groupIds: [], course: ['CET'], subject: [], topic: '', subtopic: '', startTime: null, endTime: null, instructions: '', shuffleQuestions: true, shuffleOptions: false, autoSubmitOnViolation: false, maxTabSwitches: 3, maxFocusLosses: 5, blockCopyPaste: true, requireFullscreen: false, testAccessEnabled:false, testAccessPassword:'' });
...
export function AdminTests() {
  ...
  const edit = async (testId: string) => {
    try {
      ...
      setEditingId(test._id); setPdfMode(false); setForm({ title: test.title, description: test.description || '', timingMode:test.timingMode || 'PERSONAL_DURATION', duration: test.duration || 180, negativeMarking: test.negativeMarking ?? 0.25, passingMarks: test.passingMarks ?? null, questionIds: test.questions?.map((question) => question._id) || [], groupIds: test.groups?.map((group) => group._id) || [], course: test.course || [], subject: test.subject || [], topic: test.topic || '', subtopic: test.subtopic || '', startTime: test.startTime ? new Date(test.startTime).toISOString() : null, endTime: test.endTime ? new Date(test.endTime).toISOString() : null, instructions: test.instructions || '', shuffleQuestions: test.shuffleQuestions ?? true, shuffleOptions: test.shuffleOptions ?? false, autoSubmitOnViolation: test.autoSubmitOnViolation ?? false, maxTabSwitches: test.maxTabSwitches ?? 3, maxFocusLosses: test.maxFocusLosses ?? 5, blockCopyPaste: test.blockCopyPaste ?? true, requireFullscreen: test.requireFullscreen ?? false, testAccessEnabled:test.testAccessEnabled ?? false, testAccessPassword:'' });
  ...
  const uploadPdf = async () => {
    ...
    Object.entries({ title: form.title, description: form.description, timingMode:form.timingMode, duration: form.duration, negativeMarking: form.negativeMarking, startTime: form.startTime || '', endTime: form.endTime || '', instructions: form.instructions, marksPerQuestion: 1, autoSubmitOnViolation: form.autoSubmitOnViolation, maxTabSwitches: form.maxTabSwitches, maxFocusLosses: form.maxFocusLosses, blockCopyPaste: form.blockCopyPaste, requireFullscreen: form.requireFullscreen, testAccessEnabled:form.testAccessEnabled, testAccessPassword:form.testAccessPassword }).forEach(([key, value]) => data.append(key, String(value ?? '')));
  ...
  return <>
    ...
    <Card><SectionTitle>{editingId ? 'Edit test' : pdfMode ? 'Create PDF test' : 'Create test'}</SectionTitle><Field label="Title" value={form.title} onChangeText={(title) => setForm({ ...form, title })} /><Field label="Description" value={form.description || ''} onChangeText={(description) => setForm({ ...form, description })} multiline /><Body muted>Timing mode</Body><Chips values={['PERSONAL_DURATION','FIXED_WINDOW','UNTIMED']} selected={[form.timingMode]} onChange={(values) => setForm({ ...form, timingMode:values[0] as TestPayload['timingMode'] })} /><Row><View style={{ flex: 1 }}><Field label="Personal duration (minutes)" value={String(form.duration)} onChangeText={(duration) => setForm({ ...form, duration: Number(duration) || 180 })} keyboardType="number-pad" editable={form.timingMode === 'PERSONAL_DURATION'} /></View><View style={{ flex: 1 }}><Field label="Negative Marks" value={String(form.negativeMarking)} onChangeText={(negativeMarking) => setForm({ ...form, negativeMarking: Number(negativeMarking) || 0 })} keyboardType="decimal-pad" /></View></Row><Field label={`Start Time (ISO${form.timingMode === 'FIXED_WINDOW' ? ', required' : ', optional'})`} value={form.startTime || ''} onChangeText={(startTime) => setForm({ ...form, startTime: startTime || null })} autoCapitalize="none" placeholder="2026-08-20T10:00:00+05:30" /><Field label={`End Time (ISO${form.timingMode === 'FIXED_WINDOW' ? ', required' : ', optional'})`} value={form.endTime || ''} onChangeText={(endTime) => setForm({ ...form, endTime: endTime || null })} autoCapitalize="none" /><Field label="Instructions" value={form.instructions || ''} onChangeText={(instructions) => setForm({ ...form, instructions })} multiline /><Body muted>Course</Body><Chips values={meta.courses} selected={form.course} multiple onChange={(course) => setForm({ ...form, course })} /><Body muted>Subjects</Body><Chips values={meta.allSubjects} selected={form.subject} multiple onChange={(subject) => setForm({ ...form, subject })} /><Body muted>Assigned batches</Body><Chips values={groups.map((group) => group.name)} selected={groups.filter((group) => form.groupIds.includes(group._id)).map((group) => group.name)} multiple onChange={(names) => setForm({ ...form, groupIds: groups.filter((group) => names.includes(group.name)).map((group) => group._id) })} />
      <SectionTitle>Exam controls</SectionTitle><Row wrap>{([['shuffleQuestions', 'Shuffle Questions'], ['shuffleOptions', 'Shuffle Options'], ['autoSubmitOnViolation', 'Auto-submit Violations'], ['blockCopyPaste', 'Block Copy/Paste'], ['requireFullscreen', 'Require Fullscreen'], ['testAccessEnabled', 'Require Password/PIN']] as [keyof TestPayload, string][]).map(([key, label]) => <Button key={key} title={`${form[key] ? '✓ ' : ''}${label}`} variant={form[key] ? 'primary' : 'secondary'} compact onPress={() => updateToggle(key)} />)}</Row>{form.testAccessEnabled ? <Field label={editingId ? 'New Password/PIN (blank keeps current)' : 'Password/PIN'} value={form.testAccessPassword || ''} onChangeText={(testAccessPassword) => setForm({ ...form, testAccessPassword })} secureTextEntry autoCapitalize="none" /> : null}<Row><View style={{ flex: 1 }}><Field label="Max App Switches" value={String(form.maxTabSwitches)} onChangeText={(maxTabSwitches) => setForm({ ...form, maxTabSwitches: Number(maxTabSwitches) || 3 })} keyboardType="number-pad" /></View><View style={{ flex: 1 }}><Field label="Max Focus Losses" value={String(form.maxFocusLosses)} onChangeText={(maxFocusLosses) => setForm({ ...form, maxFocusLosses: Number(maxFocusLosses) || 5 })} keyboardType="number-pad" /></View></Row>
