...
function InstructionsCard({ value, onClose, onStart }: { value: TestInstructions; onClose: () => void; onStart: () => void }) {
  return <Card><Row><Badge tone={value.availability === 'expired' ? 'danger' : value.availability === 'upcoming' ? 'warning' : 'primary'}>{value.availability.replace('_', ' ')}</Badge><Pressable onPress={onClose}><Text style={{ color: colors.primary, fontWeight: '800' }}>Close</Text></Pressable></Row><Title>{value.test.title}</Title><Body>{value.test.instructions || 'Read every question carefully. Save each answer before moving.'}</Body><DataLine label="Duration" value={`${value.test.duration} minutes`} /><DataLine label="Questions" value={value.questionCount} /><DataLine label="Total Marks" value={value.test.totalMarks} /><DataLine label="Negative Marking" value={value.test.negativeMarking ?? 0} />{value.cetSectionFlow ? <><SectionTitle>Section order</SectionTitle>{value.sectionSummary.map((section) => <DataLine key={section.subject} label={section.subject} value={`${section.questionCount} questions · ${section.totalMarks} marks`} />)}</> : null}{value.submittedResultId ? <Button title="View Submitted Result" onPress={() => router.push({ pathname: '/result/[result-id]', params: { 'result-id': String(value.submittedResultId) } })} /> : <Button title={value.inProgress ? 'Resume Exam' : 'Start Exam'} onPress={onStart} disabled={!value.canStart} />}</Card>;
}
